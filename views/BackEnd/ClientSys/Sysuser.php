...
$sqlWhere = " WHERE 1 = 1";
$sqlWhere .= " AND a.c_id = '" . $c_id . "' ";
$sqlWhere .= " AND a.user_level in  ('normal') ";
//確認使用者權限
$create = "disabled";
$read = "disabled";
$update = "disabled";
...
//取得畫面內容
$sql = "SELECT a.user_id, a.user_status, b.group_name, a.user_account, a.user_password, ";
$sql .= " a.user_name, a.user_email, a.user_phone, a.user_level ";
$sql .= $sqlFrom;
$sql .= $sqlWhere;
$initAry = $mysqli->readArraySTMT($sql, 9);
?>
<!DOCTYPE html>
                                            ...
                                            <?php foreach ($initAry as $rsAry) { ?> 
                                                <tr>
                                                    <td>
                                                        ...
                                                        <?php if ($delete == "abled") { ?><a onClick="setKey('<?php echo ENCCode($rsAry[0]); ?>');excute('delete');" class="btn btn-danger btn-sm"><i class="fa fa-trash-o"></i>刪除</a><?php } ?>
                                                    </td>
                                                    <td><?php echo $rsAry[2]; ?></td>
                                                    <td><?php echo $rsAry[3]; ?></td>
                                                    <td><?php echo $rsAry[5]; ?></td>
