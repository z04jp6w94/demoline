...
//取得畫面內容
$sql = "SELECT p.p_id, p_name, cp_name, p.entry_datetime, p.p_send_date, p.lp_type ";
$sql .= $sqlFrom;
$sql .= " left join code_push cp on cp.cp_id = p.cp_id ";
$sql .= $sqlWhere;
$sql .= " order by p.p_id desc ";
$initAry = $mysqli->readArraySTMT($sql, 6);
?>
<!DOCTYPE html>
            ...
            function excute(action) {
                switch (action) {
                    ...
                    case "update":
                        ...
                        break;
                    case "target":
                        Form2.action = '<?php echo $fileName; ?>_Target.php';
                        Form2.submit();
                        break;
                    case "clicks":
                        Form2.action = '<?php echo $fileName; ?>_Clicks.php';
                        Form2.submit();
                        break;
                }
            }
...
<?php foreach ($initAry as $rsAry) { ?> 
                                                <tr>
                                                    <td>
                                                <?php if ($update == "abled") { ?><a onClick="setKey('<?php echo $rsAry[0]; ?>');excute('update');" class="btn btn-info btn-sm"><i class="fa fa-search"></i>檢視</a><?php } ?>
    <?php if ($delete == "abled") { ?><a onClick="setKey('<?php echo $rsAry[0]; ?>');excute('delete');" class="btn btn-danger btn-sm"><i class="fa fa-trash-o"></i>刪除</a><?php } ?>
                                                    ...
                                                    <td>
                                                        <a onClick="setKey('<?php echo $rsAry[0]; ?>');excute('target');" class="btn btn-info btn-sm"><i class="fa fa-paper-plane-o"></i>再次推文</a>
                                                    </td>
                                                    <td><?php echo $rsAry[3]; ?></td>
                                                    <td><?php echo $rsAry[4]; ?></td>
                                                    <td>
    <?php if ($rsAry[5] != '1') { ?>
                                                            <a onClick="setKey('<?php echo $rsAry[0]; ?>');excute('clicks');" class="btn btn-default btn-sm"><i class="fa fa-thumbs-o-up"></i>點擊名單</a>
                                                        <?php } ?>
                                                    </td>
                                                </tr>
                                                    <?php } ?>
