...
//定義時間參數
        ...
        if ($lp_type == '2') {
            ...
            $serverFilePath = "assets_rear/images/push/" . $c_id . "/" . $_RandRoot;
            $ThumbnailSize = 250;
            $pictureFile = new BackEndPictureFileForWork($oldfileName, $newfileName, $tempFilePath, $serverFilePath, $ThumbnailSize);
            $pictureFile->createFolder("assets_rear/images/push"); //初始
            $pictureFile->createFolder("assets_rear/images/push/" . $c_id); //初始
            ...
            $UploadCDN = CDN_PUSH . $c_id . '/' . $_RandRoot . '/';
        ...
        $sql .= " ?, ?, ?, ?, ?, ";
        $sql .= " ?, '2', ?, 'N', ? )";
        $p_id = $mysqli->createPreSTMT_CreateId($sql, "sssssssssssss", array($c_id, $cp_id, $ct_str, $p_name, $p_content, $p_url, $originalImgName, $UploadCDN, $p_send_status, $p_send_datetime, $lp_type, $p_reserve, $excuteDateTime));
        //立即推播
        if ($p_send_status == '1') {
            $post_data = array("p_id" => "$p_id", "ct_str" => "$ct_str");
            $ch = curl_init(WEB_HOSTNAME . "/api/scrm/LINETargetPush/LINETargetPush_API.php");
