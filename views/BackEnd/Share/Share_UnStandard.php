...
//取得畫面內容
$sql = " SELECT sc.share_lineid, m.mlm_name, COUNT(sa_id) ";
$sql .= " FROM share_course sc ";
$sql .= " LEFT JOIN member_list_m m on m.mlm_lineid = sc.share_lineid AND sc.c_id = m.c_id ";
$sql .= " WHERE sc.sa_id = ? ";
$sql .= " AND sc.c_id = ? ";
$sql .= " AND sc.share_lineid not in (SELECT ser.mlm_lineid FROM share_exchange_record ser WHERE ser.sa_id = ? AND ser.c_id = ? )  ";
$sql .= " GROUP BY sc.share_lineid ";
$initAry = $mysqli->readArrayPreSTMT($sql, "ssss", array($dataKey, $c_id, $dataKey, $c_id), 3);
?>
<!DOCTYPE html>
    ...
    <head>
        ...
        <script type="text/javascript">
            function excute(action) {
                switch (action) {
                    case "send":
                        Form2.action = '<?php echo $fileName; ?>_EditMessage.php';
                        Form2.submit();
                        break;
                }
            }
                                ...
                                <div class="panel-body">
                                    <table id="search-textbox-table" class="table table-celled">
                                        <form name="Form2" id="Form2" method="post">
                                            ...
                                            <input type="hidden" id="sa_id" name="sa_id" value="<?php echo ENCCode($dataKey); ?>" />
                                        ...
                                        <tbody>
                                            <?php foreach ($initAry as $rsAry) { ?> 
                                                <tr>
                                                    <td><a onClick="setKey('<?php echo ENCCode($rsAry[0]); ?>');excute('send');" class="btn btn-info btn-sm"><i class="fa fa-paper-plane-o"></i>編輯訊息</a></td>
