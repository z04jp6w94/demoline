...
//取得畫面內容
$sql = " SELECT m.mlm_lineid, m.mlm_name, m.mlm_email, m.mlm_phone, ser_apply_status, ";
$sql .= " ser_status, entry_datetime, apply_datetime ";
$sql .= " FROM share_exchange_record ser ";
$sql .= " LEFT JOIN member_list_m m on m.mlm_lineid = ser.mlm_lineid AND ser.c_id = m.c_id ";
$sql .= " WHERE ser.sa_id = ? ";
$sql .= " AND ser.c_id = ? ";
    ...
    <body>
        <div class="animsition">
                        ...
                        <div class="col-lg-12">
                            <div class="panel panel-white">
                                <div class="panel-heading">
                                    <a id="add-row-btn" href="Share.php" class="btn btn-inverse"><i class="pe-7s-back"></i>返回</a>
                                        ...
                                        <tbody>
                                            <?php foreach ($initAry as $rsAry) { ?> 
                                                <tr>
                                                    <td>
                                                        <?php if ($rsAry[5] == "Y") { ?>
                                                            <a onClick="ChangeRecord('<?php echo ENCCode($rsAry[0]); ?>', 'Y');" class="btn btn-inverse btn-sm"><i class="fa fa-check"></i></a>
                                                        <?php } else { ?>
                                                            <a onClick="ChangeRecord('<?php echo ENCCode($rsAry[0]); ?>', 'N');" class="btn btn-inverse btn-sm"><i class="fa fa-close"></i></a>
                                                        <?php } ?>
                                                    </td>
                                                    ...
                                                    <td>
                                                        <?php if ($rsAry[4] == "Y") { ?>
                                                            <a class="btn btn-inverse btn-sm"><i class="fa fa-check"></i></a>
                                                        <?php } else { ?>
                                                            <a class="btn btn-inverse btn-sm"><i class="fa fa-close"></i></a>
                                                            <?php } ?>
                                                    </td>
                                                    <td><?php echo $rsAry[7]; ?></td>
                                                </tr>
                                            <?php } ?>
                                        </tbody>
                                    </table>
            ...
            <script type="text/javascript">
                                                        function ChangeRecord(lineid, status) {
                                                            var c_id = '<?php echo $c_id; ?>';
                                                            var sa_id = '<?php echo $dataKey; ?>';
                                                            $.post(
                                                                    'Share_Ajax.php',
                                                                    {c_id: c_id, sa_id: sa_id, lineid: lineid, status: status},
                                                                    function (data) {

                                                                    }
                                                            );

                                                            Form2.action = '<?php echo $fileName; ?>_Standard.php';
                                                            Form2.submit();
                                                        }
            </script>
        </div>
