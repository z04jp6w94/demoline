...
/*  */
$sqlFrom = " FROM commodity_m cm ";
$sqlWhere = " WHERE 1 = 1 ";
$sqlWhere .= " AND cm.c_id = '" . $c_id . "' ";
...
//取得畫面內容
$sql = " SELECT cm.cm_id, cm_name, cc.cc_name, cm_price, cm.cm_starttime, ";
$sql .= " cm.cm_endtime ";
$sql .= $sqlFrom;
$sql .= " LEFT JOIN code_commodity cc ON cc.cc_id = cm.cc_id ";
$sql .= $sqlWhere;
$sql .= " ORDER BY cm.cm_id DESC ";
                                    ...
                                    <table id="search-textbox-table" class="table table-celled">
                                        ...
                                        <tbody>
                                            <?php foreach ($initAry as $rsAry) { ?> 
                                                <tr>
                                                    <td>
                                                        <?php if ($update == "abled") { ?><a onClick="setKey('<?php echo $rsAry[0]; ?>');excute('update');" class="btn btn-info btn-sm"><i class="fa fa-search"></i>編輯</a><?php } ?>
