...
if ($push_type == "3" or $push_type == "5") {
    ...
    for ($i = 0; $i < count($fpmdAry); $i++) {
        /* 刪除檔案 原圖,縮圖 */
        if ($fpmdAry[$i][1] != '') {
            $sql = "SELECT count(fpmd_id) ";
            $sql .= " FROM fb_push_messenger_d ";
            $sql .= " WHERE fpmd_push_type = ? ";
            $sql .= " AND fpmd_img = ? ";
            $sql .= " AND c_id = ? ";
            $sql .= " AND deletestatus = 'N' ";
            $fpmd_count = $mysqli->readValuePreSTMT($sql, "sss", array($push_type, $fpmdAry[$i][1], $c_id));
            if ($fpmd_count > 1) {
                break;
            } else {
    ...
    /* 刪除檔案 原圖,縮圖 */
    if ($initAry[0][1] != '') {
        $sql = "SELECT count(fpm_id) ";
        $sql .= " FROM fb_push_messenger ";
        $sql .= " WHERE fpm_push_type = ? ";
        $sql .= " AND fpm_img = ? ";
        $sql .= " AND c_id = ? ";
        $sql .= " AND deletestatus = 'N' ";
        $fpm_count = $mysqli->readValuePreSTMT($sql, "sss", array($push_type, $initAry[0][1], $c_id));
        if ($fpm_count > 1) {
