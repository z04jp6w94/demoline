...
$sql = "SELECT c.c_name, p.p_name, cbr_price, cbr_date, cbr_st_date, cbr_end_date ";
$sql .= " FROM crm_buy_records cbr ";
$sql .= " left join crm_m c on c.c_id = cbr.c_id ";
$sql .= " left join product_m p on p.p_number = cbr.p_number ";
$sql .= " WHERE cbr_id = ?";
$initAry = $mysqli->readArrayPreSTMT($sql, "s", array($dataKey), 6);
$sql = " select c_id, c_name from crm_m ";
$sql .= " where c_id != '0000000000' ";
                            ...
                            <form name="form1" id="form1" method="post" action="<?php echo $fileName; ?>_Update.php" autocomplete="off">
                                ...
                                <div class="panel panel-white">
                                    ...
                                    <div class="panel-body">
                                        <div>
                                            <h5>客戶</h5>
                                            <?php echo $initAry[0][0]; ?>                                           
                                            <hr class="full-width" /> 
                                            <h5>產品</h5>
                                            <?php echo $initAry[0][1]; ?>   
                                            <hr class="full-width" />
                                            <label for="definput"><b>購買價格</b></label>
                                            <input type="text" class="form-control" id="cbr_price" name="cbr_price" value="<?php echo $initAry[0][2]; ?>" onKeyUp="return this.value = this.value.replace(/\D/g, '')" placeholder="請輸入購買價格" autocomplete="off" title="請輸入購買價格" required />
                                            <hr class="full-width" />                                                                                
                                            <label for="definput"><b>購買時間</b></label>
                                            <div class="input-group">
                                                <input class="form-control" id="date-picker" name="cbr_date" value="<?php echo substr($initAry[0][3], 0, 10); ?>" type="text" readonly data-date-format="yyyy/mm/dd">
                                            ...
                                            <div class="input-group">
                                                ...
                                                <input type="text" value="<?php echo str_replace("-", "/", substr($initAry[0][4], 0, 10)); ?> - <?php echo str_replace("-", "/", substr($initAry[0][5], 0, 10)); ?>" name="cbr_date_range" class="form-control" id="daterangepicker" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
