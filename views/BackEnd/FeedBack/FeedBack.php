...
/*  */
$sqlFrom = " FROM feedback f ";
$sqlFrom .= " left join member_list_m mlm on mlm.mlm_lineid = f.mlm_lineid and mlm.c_id = f.c_id  ";
$sqlWhere = " WHERE 1 = 1 ";
$sqlWhere .= " AND f.c_id = '" . $c_id . "' ";
//確認使用者權限
$create = "disabled";
$read = "disabled";
$update = "disabled";
...
//取得畫面內容
$sql = "SELECT f_id, f_problem, f_status, f.entry_datetime, mlm.mlm_name,  ";
$sql .= " (SELECT fc_response FROM feedback_course fc WHERE fc.fc_id = f.f_id limit 1) AS 'LAST ANSWER', ";
$sql .= " (SELECT modify_datetime FROM feedback_course fc WHERE fc.fc_id = f.f_id limit 1) AS 'LAST TIME' ";
$sql .= $sqlFrom;
$sql .= $sqlWhere;
                                    ...
                                    <table id="search-textbox-table" class="table table-celled">
                                        ...
                                        <tbody>
                                            <?php foreach ($initAry as $rsAry) { ?> 
                                                <tr>
                                                    <td><?php if ($update == "abled") { ?><a onClick="setKey('<?php echo $rsAry[0]; ?>');excute('update');" class="btn btn-info btn-sm"><i class="fa fa-edit"></i>回覆</a><?php } ?></td>
                                                    <td><?php echo $rsAry[4]; ?></td>
                                                    <td>
                                                        <?php if($rsAry[2] == "3"){?>
                                                            <a class="btn btn-success btn-sm"><i class="fa fa-check"></i>已回覆</a>
                                                        <?php }else if($rsAry[2] == "2"){?>
                                                            <a class="btn btn-warning btn-sm"><i class="fa fa-clock-o"></i>處理中</a>
                                                        <?php }else if($rsAry[2] == "1"){?>
                                                            <a class="btn btn-danger btn-sm"><i class="fa fa-close"></i>尚未回覆</a>
                                                        <?php }?>                                                        
                                                    </td>
                                                    <td><?php echo $rsAry[3]; ?></td>
                                                    <td><?php echo $rsAry[6]; ?></td>
                                                </tr>
                                            <?php } ?>
                                        </tbody>
                                    </table>
