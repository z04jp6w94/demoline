...
//定義時間參數
    ...
    if ($rsm_type == '1') {
        ...
        for ($i = 0; $i < 6; $i++) {
            ...
            if ($rst_type == '1') {
                $action_type = 'message';
                $action_type_option = 'text';
                $action = $_REQUEST["lrcm_keyword"][$i];
            } else if ($rst_type == '2') {
                $action_type = 'uri';
        ...
        if ($rst_type == '1') {
            $action_type = 'message';
            $action_type_option = 'text';
            $action = $_REQUEST["lrcm_keyword"][0];
        } else if ($rst_type == '2') {
            $action_type = 'uri';
            $action_type_option = 'uri';
    ...
    if ($useValue == "1") {
        ...
        for ($i = 0; $i < count($mlmAry); $i++) {
            $User = $mlmAry[$i][0];
            $UserStatus = $LineRichMenu->LinkMenuToUser($richMenuId, $User);
            if (!$UserStatus) {
                error_log($c_id . "->" . $dataKey . "->" . $User . ": RichMenu FALSE!");
            }
        }
        $rsm_status = "Y";
