...
//        $("#ischange_p_img").val("Y");
        $('#uploadFileChooseButton' + number).html("<input type='file' name='lrcm_img[]' id='lrcm_img" + number + "' class='upload' style='display:none;'></input>選擇檔案");
    ...
    $(document).on('click', '.action_type', function () {
        ...
        at_type = this.value;
        if (at_type == '1') {
            $("#title" + at_number).show(); //主旨
            $("#content" + at_number).show(); //內容
            $("#keyword" + at_number).show(); //應用程式- 關鍵字
            $("#url" + at_number).hide(); //應用程式- 超連結
            $("#lrcm_url" + at_number).val(""); //應用程式- 關鍵字
            $("#commodity" + at_number).hide(); //應用程式- 商品
            $("#img_" + at_number).show(); //圖片
...
function chkFormField(createForm) {
    ...
    if (_lrcm_type == 1) {
        ...
        if (FormDataEmptyTooltip(createForm.lrcm_keyword)) {
            $('#lrcm_keyword').tooltip().attr('data-original-title', '請輸入關鍵字');
            $('#lrcm_keyword').tooltip("show");
            $('#lrcm_keyword').focus();
            _form_status = false;
        } else if (FormDataEmptyTooltip(createForm.lrcm_title)) {
            $('#lrcm_title').tooltip().attr('data-original-title', '請輸入主旨');
            $('#lrcm_title').tooltip("show");
            $('#lrcm_title').focus();
            _form_status = false;
        } else if (FormDataEmptyTooltip(createForm.lrcm_content)) {
        ...
        if (_form_status) {
            ...
            if (radio_check.val() == '1') {
                if (FormDataEmptyTooltip(createForm.lrcm_title)) {
                    $('#lrcm_title').tooltip().attr('data-original-title', '請輸入主旨');
                    $('#lrcm_title').tooltip("show");
                    $('#lrcm_title').focus();
                    _form_status = false;
    ...
    if (_form_status) {
        $.post(
                'LINERichMenuContent_Check.php',
                {keyword: $('#lrcm_keyword').val()},
                function (data) {
